...
class Runner:
	...
	def get_predictions(self, model, tensor_examples, stored_info):
		model.to(self.device)
		evaluator = CorefEvaluator()
		doc_to_prediction = {}
	...
	def load_model_checkpoint(self, model, suffix):
		path_ckpt = join(self.config['model_path'], f'model_{suffix}.bin')
		print("################################################")
...
if __name__ == '__main__':
	...
	if len(sys.argv) > 4:
		config_name, gpu_id, seed, re_train = sys.argv[1], int(sys.argv[2]), int(sys.argv[3]), str(sys.argv[4])
	else:
		config_name, gpu_id, seed, re_train = sys.argv[1], int(sys.argv[2]), int(sys.argv[3]), None

	runner = Runner(config_name, gpu_id, seed)

	if re_train:
		model = runner.initialize_model()
		model_name = runner.config["bert_pretrained_name_or_path"]
		embedding_name = re_train
		path_ckpt = f"embedding_models/{model_name}/{embedding_name}/model.pt"
		model.bert.load_state_dict(torch.load(path_ckpt))
		print("################################################")
		logger.info(f"LOADED FEATURE EXTRACTOR CHECKPOINT FROM {path_ckpt}")
	else:
		model = runner.initialize_model()
