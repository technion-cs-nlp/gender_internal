...
def minimize_partition(partition, extension, args, tokenizer):
	input_path = os.path.join(args.input_dir, f'{partition}_{args.name}.{extension}')
	output_path = os.path.join(args.output_dir, f'{partition}_{args.name}.{args.seg_len}.jsonlines')
	doc_count = 0
...
def minimize_language(args):
	...
	minimize_partition('dev', "v4_auto_conll", args, tokenizer)
	minimize_partition('test', "v4_auto_conll", args, tokenizer)
